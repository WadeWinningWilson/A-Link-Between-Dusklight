...
//   canALBWHiddenSkill: requires a completely full meter (>= sOilMaxVar) —
//     Great Spin, Jump Strike, Helm Splitter, Mortal Draw may not be used
//     until the player has let the meter fully recover after any depletion.
// ============================================
...
// ============================================
// NEW CODE — ALBW Port
// Sword and agility gates check only the lockout latch, not a minimum
// meter amount.  This lets the action execute even when the remaining
// meter is less than its own drain cost — the drain may push the meter
// below zero, clamping it to 0 and latching sALBWLocked, after which
// checkRestHPAnime() triggers tired state at the end of the animation.
// This matches the design intent: "six attacks drain the meter, tired
// state enters after the animation ends."  A per-cost threshold would
// block the final action before the meter reaches zero, breaking the
// intended six-swing cycle.
//
// Hidden skill gate is intentionally different: those moves require a
// fully charged meter (>= sOilMaxVar) as an explicit design constraint.
// ============================================
...
bool dMeter2_canALBWHiddenSkill() { return !sALBWLocked && sALBWMeter >= sOilMaxVar; }
// ============================================
// NEW CODE — ALBW Port
...
// sALBWRentalEligible[]:            One entry per ALBW item (12 total).
//   Set to true the first time that item is stripped on death; persists
//   in-session so the rental shop knows what was ever acquired.
// sALBWItemNos[]:                   Item-number lookup matching the
//   rental eligible array. Order: Boomerang, Spinner, Bow, Ball&Chain,
//   DomRod, Clawshot, DClawshot, Normal Bombs, Water Bombs, Bomblings,
//   Slingshot, Magic Armor.
// dMeter2_onALBWRentalEligible():   Marks an item as rental-eligible by
//   item number. Must be called BEFORE offItemFirstBit so the record
//   survives the strip.
// dMeter2_isALBWRentalEligible():   Queried by the rental shop to decide
//   whether to show the item or "Not in stock — Come Back Soon!".
// ============================================
static bool sALBWRentalEligible[12] = {};
...
void dMeter2_c::moveKantera() {
    ...
    // ============================================
    {
        ...
        // ============================================
        const int sRecoveryRate = 109 + sPrevUnlockCount * 55;
        // ============================================
        // NEW CODE ENDS HERE
        // ============================================
